...
PRIVATE struct
{
    ...
    { 0, TICK, "TICK", 4096, 4096, tick_main },
    { 1, MM, "MM", 4096, 4096, mm_main },
    { 0, VIEW, "VIEW", 4096, 4096, view_main },
    { 1, USB_SRV, "USB Service", 4096, 4096, usb_main },
    { 1, KBD_SRV, "Keyboard Services", 4096, 4096, keyboard_main },
};
...
PUBLIC void service_init(void)
{
    ...
    for (i = 0; i < SERVICES; i++)
    {
        ...
        size_t         ustack_size = services[i].ustack_size;
        void          *func        = services[i].func;
        if (services[i].kernel_task)
        {
            task = task_start(name, prio, kstack_size, func, 0);
        }
        else
        {
            task = proc_execute(name, prio, kstack_size, ustack_size, func);
        }
