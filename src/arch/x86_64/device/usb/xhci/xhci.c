...
PRIVATE int xhci_hub_reset(usb_hub_t *hub,uint32_t port)
{
    ...
    {
        pr_log("\3 Port not connected before reset.\n");
        return -1;
    }
...
PUBLIC status_t xhci_setup(usb_hub_set_t *hub_set)
{
    ...
    if (ERROR(status))
    {
        pr_log("\3 Failed to alloc hub_set.\n");
        return status;
    }
    ...
    int timeout)
{
    while (fifo_empty(fifo))
    {
        ...
        if (--timeout == 0)
        {
            pr_log("\3 Process command timeout.\n");
            return CC_INVALID;
        }
    }
    // Completed
...
PRIVATE xhci_in_ctx_t *xhci_alloc_in_ctx(usb_device_t *usb_dev,int max_epid)
{
    ...
    if (ERROR(status))
    {
        pr_log("\3 Failed to alloc in ctx.\n");
        return NULL;
    }
...
PRIVATE int xhci_configure_hub(usb_hub_t *hub)
{
    ...
    if (cc != CC_SUCCESS)
    {
        pr_log("\3 Failed to configure ep.\n");
        return -1;
    }
    return 0;
}
    ...
    usb_endpoint_descriptor_t *epdesc)
{
    ...
    if (ERROR(status))
    {
        pr_log("\3 Failed to alloc pipe reqs.trbs.\n");
    ...
    if (in_ctx == NULL)
    {
        pr_log("\3 Failed to alloc in_ctx.\n");
        goto fail;
    }
    ...
    if (pipe->epid == 1)
    {
        ...
        if (slot_id < 0)
        {
            pr_log("\3 Failed to enable slot.\n");
            pfree(dev);
            goto fail;
        }
        ...
        if (cc != CC_SUCCESS)
        {
            pr_log("\3 Failed to address device: cc = %d.\n",cc);
            cc = xhci_cmd_disable_slot(xhci,slot_id);
            if (cc != CC_SUCCESS)
            {
                pr_log("\3 Failed to disable slot.\n");
                goto fail;
            }
    ...
    usb_endpoint_descriptor_t *epdesc)
{
    ...
    if (cc != CC_SUCCESS)
    {
        pr_log("\3 Failed to evaluate context.\n");
    }
    pfree(KADDR_V2P(in_ctx));
    return upipe;
}
