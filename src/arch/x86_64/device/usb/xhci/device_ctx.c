    ...
    uint16_t using_64byte_ctx)
{
    ...
    memset(ctrl_ep_ring,0,sizeof(xhci_trasfer_ring_t));
    ...
    status = pmalloc(sizeof(ctrl_ep_ring->ring[0]) * XHCI_TRANSFER_RING_COUNT,
                     XHCI_TRANSFER_RING_SEGMENTS_ALIGNMENT,
                     XHCI_TRANSFER_RING_SEGMENTS_BOUNDARY,
                     &transfer_ring);
    if (ERROR(status))
    ...
    memset(
        ctrl_ep_ring->ring,
        0,
        sizeof(ctrl_ep_ring->ring[0]) * XHCI_TRANSFER_RING_COUNT);
