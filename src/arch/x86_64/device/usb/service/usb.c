...
PRIVATE int speed_to_ctrl_size(uint8_t speed)
{
    ...
    {
        case USB_FULLSPEED:
        case USB_LOWSPEED:   return 8;
        case USB_HIGHSPEED:  return 64;
        case USB_SUPERSPEED: return 512;
    }
    return -1;
};
    ...
    usb_endpoint_descriptor_t *epdesc)
{
    ...
    {
        if (period >> 8) { period >>= 8; ret += 8; }
        if (period >> 4) { period >>= 4; ret += 4; }
        if (period >> 2) { period >>= 2; ret += 2; }
        if (period >> 1) { ret += 1; }
...
PRIVATE int usb_set_address(usb_device_t *usb_dev)
{
    ...
    if (ret != 0)
    {
        pr_log("\3 failed to send defaule control.\n");
...
PRIVATE int configure_usb_device(usb_device_t *usb_dev)
{
    ...
    if (ret != 0)
    {
        pr_log("\3 Failed to get device info.\n");
        return 0;
    }
    ...
    pr_log("\1 device rev=%04x cls=%02x sub=%02x proto=%02x size=%d\n",
        dinfo.bcdUSB,
        dinfo.bDeviceClass,
...
PRIVATE int usb_hub_port_setup(usb_device_t *usb_dev)
{
    ...
    if (ret < 0)
    {
        pr_log("\3 Port reset fail.ret=%d\n",ret);
        goto reset_fail;
    }
    usb_dev->speed = ret;
    ...
    if (ret < 0)
    {
        pr_log("\3 usb set address failed.\n");
        hub->op->disconnect(hub,port);
        goto reset_fail;
    }
    pr_log("\1 usb set address success.\n");

    pr_log("\1 Configure the device.\n");
...
PUBLIC void usb_main(void)
{
    ...
    usb_setup(&xhci_hubs);
    pr_log("\2 Hub set at %p,count=%d.\n",xhci_hubs.hubs,xhci_hubs.count);
    task_start("XHCI",DEFAULT_PRIORITY,4096,xhci_event,(uint64_t)&xhci_hubs);
    task_yield();
    ...
    while (1)
    {
        ...
        for (i = 0;i < xhci_hubs.count;i++)
        {
            ...
            if (pc_evt.is_connect)
            {
                ...
                if (ret < 0)
                {
                    pr_log("\3 Failed to setup port: %d.\n",pc_evt.port);
                }
                else
                {
                    pr_log("\1 setup port successful: %d.\n",pc_evt.port);
                }
            }
            else
            {
