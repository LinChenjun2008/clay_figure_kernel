...
PRIVATE uint32_t
pci_config_read(uint8_t bus, uint8_t dev, uint8_t func, uint8_t offset)
{
    uint32_t address;
    /// PCI_CONFIG_ADDRESS:
    ...
    address = 0x80000000 | (uint32_t)bus << 16 | (uint32_t)dev << 11 |
...
PRIVATE uint8_t
pci_read_secondary_bus_number(uint8_t bus, uint8_t dev, uint8_t func)
{
...
PUBLIC uint64_t pci_dev_read_bar(pci_device_t *dev, uint8_t bar_index)
{
    ...
    return ((uint64_t)pci_dev_config_read(dev, offset + 4) << 32) |
           (bar & mask);
}
