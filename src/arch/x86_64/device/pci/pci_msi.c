...
)
{
    ...
    if (msi_cap_addr)
    {
        ...
        uint32_t msi_pending_bits =
            pci_dev_config_read(dev, msi_cap_addr + 0x14);
...
pci_dev_configure_msi(pci_device_t *dev, uint32_t irq, uint32_t count)
{
    if (!dev->msi.msi_capable)
    {
        pr_log(
            LOG_ERROR,
            "PCI Device %x:%x:%x Not Support MSI.\n",
            dev->bus,
            dev->device,
            dev->func
        );
    ...
    msi->data_value    = irq;
    pci_dev_config_write(
        dev, msi->cap_addr + 0x04, msi->address_value & 0xffffffff
    );
    if (msi->msg_ctrl & 0x0080) // 64bit
    {
        pci_dev_config_write(
            dev, msi->cap_addr + 0x08, msi->address_value >> 32
        );
